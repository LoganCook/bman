...
# ersaDynamics is still in private local repositiory, you have to copy source to the host and run:
pip install -e ersaDynamics
deactivate
...
ExecStart=$PDIR/env/bin/gunicorn --error-logfile /var/log/gunicorn/dynamicsp_error.log --log-level info -b 127.0.0.1:8001 runner.dynamicsp
